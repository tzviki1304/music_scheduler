...
class MusicSchedulerApp:
    ...
    def _create_schedule_section(self, parent):
        ...
        days_vars = {}
        for day in days:
            days_vars[day] = tk.BooleanVar()
            day_check = ttk.Checkbutton(days_frame, text=day, variable=days_vars[day])
    ...
    def add_schedule(self, hours, minutes, days_vars):
        ...
        try:
            ...
            messagebox.showerror("שגיאה", "אנא הזן שעה וזמן תקינים")
            return
        ...
        # Add the schedule
        try:
            self.music_scheduler.add_schedule(time_str, selected_days)
            self.schedules_listbox.insert(tk.END, f"{time_str} - {', '.join(selected_days)}")
            self.save_settings()
    ...
    def load_settings(self):
        ...
        try:
            ...
            for schedule in schedules:
                ...
                if time_str:
                    self.music_scheduler.add_schedule(time_str, days)
                    self.schedules_listbox.insert(tk.END, f"{time_str} - {', '.join(days)}")
